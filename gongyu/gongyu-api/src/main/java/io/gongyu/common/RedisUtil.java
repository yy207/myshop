...
public class RedisUtil {
	 ...
	 * 存储在Redis第二个数据库里
	 */
	public void jedisSetTime(String mobile) {
		Jedis jedis = new Jedis("localhost",6379);
		jedis.select(1);
		jedis.set(mobile,"已修改");
		//设置过期时间一年
		jedis.expire(mobile,31536000);
	}
	/**
	 * 	get
	 * 获取
	 * 存储在Redis第二个数据库里
	 */
	public String jedisGetTime(String mobile) {
		Jedis jedis = new Jedis("localhost",6379);
		jedis.select(1);
		String mobiles = jedis.get(mobile);
		return mobiles;
	}
	/**
	 * 区域修改
	 *
	 */
	public void cityArea(String mobile) {
		securityCode(mobile,"已修改",60,TimeUnit.DAYS);
	}
	/**
	 * 验证码
	 */
	public void securityCode(String key, String value,long timeout, TimeUnit unit) {
